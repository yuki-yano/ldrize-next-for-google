...
export const ldrizeSlice = createSlice({
  ...
  reducers: {
    ...
    updateCandidates: (
      state,
      action: PayloadAction<{ candidates: Array<Candidate> }>
    ) => {
      const links = state.candidates.map((candidate) => candidate.link);
      const candidates = action.payload.candidates.filter(
        (candidate) => !links.includes(candidate.link)
      );
    ...
    tabOpen: (state) => {
      ...
      if (candidates.length === 0) {
        const url = state.candidates.find(
